...
__device__ __forceinline__ ReduceResult reduce_max_and_sum(float* arr, int tid) {
	...
	for (int i = 0; i < 128; i += 8) {
	        ...
	        myMax = max(myMax, max(max(max(max(max(max(max(a0, a1), a2), a3), a4), a5), a6), a7));
	        mySum += __expf(a0 - myMax) + __expf(a1 - myMax) + __expf(a2 - myMax) + __expf(a3 - myMax) + __expf(a4 - myMax) + __expf(a5 - myMax) + __expf(a6 - myMax) + __expf(a7 - myMax);
	}
...
__global__ void softmax(float* out, float* in) {
	...
	for (int i = 0; i < 128; i += 8) {
        	out[blockIdx.x * N + i * BLOCK_SIZE + threadIdx.x] = __expf(in[blockIdx.x * N + threadIdx.x] - res.max) / res.sum;  // NOTE：重复计算
        	out[blockIdx.x * N + (i + 1) * BLOCK_SIZE + threadIdx.x] = __expf(in[blockIdx.x * N + BLOCK_SIZE + threadIdx.x] - res.max) / res.sum;  // NOTE：重复计算
        	out[blockIdx.x * N + (i + 2) * BLOCK_SIZE + threadIdx.x] = __expf(in[blockIdx.x * N + 2 * BLOCK_SIZE + threadIdx.x] - res.max) / res.sum;  // NOTE：重复计算
        	out[blockIdx.x * N + (i + 3) * BLOCK_SIZE + threadIdx.x] = __expf(in[blockIdx.x * N + 3 * BLOCK_SIZE + threadIdx.x] - res.max) / res.sum;  // NOTE：重复计算
        	out[blockIdx.x * N + (i + 4) * BLOCK_SIZE + threadIdx.x] = __expf(in[blockIdx.x * N + 4 * BLOCK_SIZE + threadIdx.x] - res.max) / res.sum;  // NOTE：重复计算
        	out[blockIdx.x * N + (i + 5) * BLOCK_SIZE + threadIdx.x] = __expf(in[blockIdx.x * N + 5 * BLOCK_SIZE + threadIdx.x] - res.max) / res.sum;  // NOTE：重复计算
        	out[blockIdx.x * N + (i + 6) * BLOCK_SIZE + threadIdx.x] = __expf(in[blockIdx.x * N + 6 * BLOCK_SIZE + threadIdx.x] - res.max) / res.sum;  // NOTE：重复计算
       		out[blockIdx.x * N + (i + 7) * BLOCK_SIZE + threadIdx.x] = __expf(in[blockIdx.x * N + 7 * BLOCK_SIZE + threadIdx.x] - res.max) / res.sum;  // NOTE：重复计算
	}
}
